...
function EmissionFactory(config, amqp) {	
	...
	this._isJSON = (config.format === "json"); //Default should be BSON
		...
		while(emission && conns.length > 0) {
			...
			if(conn.isConnected()) {
				...
				if(this._isJSON) {
					...
					content = JSON.stringify(emission);
				}
				else {
					contentType = "application/bson";
					content = BSON.serialize(emission);
				}
				conn.publish(emission.t, content, headers, contentType);
